...
export type TabKey = 'mushaf' | 'topics' | 'bookmarks' | 'stats' | 'tafsir' | 'quiz' | 'qiraat' | 'hijri' | 'voice' | 'kids' | 'a11y' | 'khatma' | 'reflections' | 'mpquiz' | 'aitafsir' | 'quranmap' | 'publicapi' | 'admin';
...
export default function Home() {
  ...
  const [tafsirVerse, setTafsirVerse] = useState<{ surah: number; ayah: number; text: string } | null>(null);
  const [shareVerse, setShareVerse] = useState<{ surah: number; ayah: number; text: string; topicColor: string; topicId: number } | null>(null);
  const [reflectVerse, setReflectVerse] = useState<{ surah: number; ayah: number; text: string; topicColor: string; topicId: number; page: number | null } | null>(null);
  const [aiTafsirVerse, setAiTafsirVerse] = useState<{ surah: number; ayah: number; text: string; topicColor: string; topicId: number } | null>(null);
  ...
  return (
    <div className="flex h-dvh overflow-hidden">
      ...
      <Sidebar
        ...
        onGoToPage={(p: number) => { goToPage(p); setSidebarOpen(false); }}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main id="main-content" className="flex-1 overflow-y-auto" role="tabpanel" aria-label={activeTab}>
          {activeTab === 'mushaf' && (
            <>
          ...
          {activeTab === 'topics' && (
            <TopicsIndex
              onGoToPage={goToPage}
              onFilterTopic={(color) => { setFilterTopic(color); setActiveTab('mushaf'); }}
            />
          )}
          {activeTab === 'bookmarks' && (
            <BookmarkPanel onGoToPage={goToPage} />
          )}
          {activeTab === 'stats' && (
            <StatsPanel onGoToPage={goToPage} />
          )}
          {activeTab === 'tafsir' && tafsirVerse && (
            <TafsirPanel
              ...
              onNavigate={(s, a) => setTafsirVerse({ surah: s, ayah: a, text: '' })}
            />
          )}
          {activeTab === 'tafsir' && !tafsirVerse && (
            <div className="flex flex-col items-center justify-center h-64 gap-4 text-center">
              <p className="text-lg text-[var(--color-mushaf-text)]/60">اضغط على آية أولاً لعرض التفسير</p>
              <button
                onClick={() => setActiveTab('mushaf')}
                className="px-4 py-2 bg-[var(--color-mushaf-gold)] text-white rounded-lg hover:opacity-90"
              >
                العودة للمصحف
              </button>
            </div>
          ...
          {activeTab === 'quiz' && (
            <QuizPanel onGoToPage={goToPage} />
          )}
          {activeTab === 'qiraat' && (
            <QiraatPanel onGoToPage={goToPage} />
          )}
          {activeTab === 'hijri' && (
            <HijriCalendarPanel onGoToPage={goToPage} />
          )}
          {activeTab === 'voice' && (
            <VoiceSearchPanel onGoToPage={goToPage} />
          )}
          {activeTab === 'kids' && (
            <KidsModePanel onGoToPage={goToPage} />
          )}
          {activeTab === 'a11y' && (
            <AccessibilityPanel />
          )}
          {activeTab === 'khatma' && (
            <KhatmaPlannerPanel onGoToPage={goToPage} />
          )}
          {activeTab === 'reflections' && (
            <ReflectionsPanel
              onGoToPage={goToPage}
              verseContext={reflectVerse || undefined}
            />
          )}
          {activeTab === 'mpquiz' && (
            <MultiplayerQuizPanel onGoToPage={goToPage} />
          )}
          {activeTab === 'aitafsir' && (
            <AiTafsirPanel
              onGoToPage={goToPage}
              verseContext={aiTafsirVerse || undefined}
            />
          )}
          {activeTab === 'quranmap' && (
            <QuranMapPanel onGoToPage={goToPage} />
          )}
          {activeTab === 'publicapi' && (
            <PublicApiPanel />
          )}
          {activeTab === 'admin' && (
            <AdminDashboard onGoToPage={goToPage} />
          )}
